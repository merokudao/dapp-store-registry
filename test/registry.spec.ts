...
describe('DappStoreRegistry', () => {
  ...
  describe('Registry Strategy', () => {
    ...
    before(() => {
      ...
      localRegistryJson = JSON.parse(fs.readFileSync('./src/registry.json').toString());
    });
    ...
    it('uses the local registry file upon Static RegistryStrategy', async () => {
      ...
      (await registry.getRegistryTitle()).should.equal(localRegistryJson1.title);

      const listedDapps = localRegistryJson1.dapps.filter(dapp => dapp.isListed);
  ...
  describe('#dApps', () => {
    ...
    it('is able to filter results on chainId', async () => {
      ...
      const dapps_1 = fixtureRegistryJson.dapps.filter(dapp => dapp.chains.includes(1));
    ...
    it('is able to filter results on language', async () => {
      ...
      const dapps_1 = fixtureRegistryJson.dapps.filter(dapp => dapp.language === language);
      registry_dapps_1.should.deep.equal(dapps_1);
    });
    ...
    it('is able to filter results on age', async () => {
      ...
      const dapps_1 = fixtureRegistryJson.dapps.filter(dapp => dapp.minAge > age);
      registry_dapps_1.should.deep.equal(dapps_1);
    });
    ...
    it('is able to filter results on matureAudience flag', async () => {
      const registry_dapps_1 = await registry.dApps({forMatureAudience: true});
      const dapps_1 = fixtureRegistryJson.dapps.filter(dapp => dapp.isForMatureAudience);
      registry_dapps_1.should.deep.equal(dapps_1);

      const registry_dapps_2 = await registry.dApps({forMatureAudience: false});
      const dapps_2 = fixtureRegistryJson.dapps.filter(dapp => !dapp.isForMatureAudience);
      registry_dapps_2.should.deep.equal(dapps_2);
    });

    it('is able to filter results on specific developer', async () => {
      const registry_dapps_1 = await registry.dApps({developer: {
        githubID: 'bitpack1'
      }});
      const dapps_1 = fixtureRegistryJson.dapps.filter(dapp => dapp.developer?.githubID === 'bitpack1');
      registry_dapps_1.should.deep.equal(dapps_1);
    });
    ...
    it('is able to filter results on category', async () => {
      ...
      const dapps_1 = fixtureRegistryJson.dapps.filter(dapp => categories.includes(dapp.category));
      registry_dapps_1.should.deep.equal(dapps_1);
    });

    it('is able to filter results on platform', async () => {
      let platforms = ['ios'];
      const registry_dapps_1 = await registry.dApps({availableOnPlatform: platforms});
      const dapps_1 = fixtureRegistryJson.dapps.filter(dapp => dapp.availableOnPlatform.some(x => platforms.includes(x)));
      registry_dapps_1.should.deep.equal(dapps_1);

      platforms = ['ios', 'android'];
      const registry_dapps_2 = await registry.dApps({availableOnPlatform: platforms});
      const dapps_2 = fixtureRegistryJson.dapps.filter(dapp => dapp.availableOnPlatform.some(x => platforms.includes(x)));
      registry_dapps_1.should.deep.equal(dapps_1);
    });

    it('is able to filter results on allowed countries', async () => {
      let validCountries = ['IN'];
      const registry_dapps_1 = await registry.dApps({allowedInCountries: validCountries});
      const dapps_1 = fixtureRegistryJson
        .dapps
        .filter((dapp) => {
          return dapp.geoRestrictions
            ?.allowedCountries
            ?.some(x => validCountries.includes(x));
        });
      registry_dapps_1.should.deep.equal(dapps_1);

      validCountries = ['AU', 'US'];
      const registry_dapps_2 = await registry.dApps({allowedInCountries: ['AU', 'US']});
      const dapps_2 = fixtureRegistryJson.dapps
      .filter((dapp) => {
        return dapp.geoRestrictions
          ?.allowedCountries
          ?.some(x => validCountries.includes(x));
    ...
    it('is able to filter results on blocked countries', async () => {
      ...
      const registry_dapps_1 = await registry.dApps({blockedInCountries: invalidCountries});
      const dapps_1 = fixtureRegistryJson
        .dapps
        .filter((dapp) => {
          return dapp.geoRestrictions
            ?.blockedCountries
            ?.some(x => invalidCountries.includes(x));
        });
      registry_dapps_1.should.deep.equal(dapps_1);

      invalidCountries = ['RU'];
      const registry_dapps_2 = await registry.dApps({blockedInCountries: invalidCountries});
      const dapps_2 = fixtureRegistryJson.dapps
      .filter((dapp) => {
        return dapp.geoRestrictions
          ?.blockedCountries
    ...
    it('is able to filter results on or after certain list date', async () => {
      ...
      const registry_dapps_1 = await registry.dApps({listedOnOrAfter: startDate});
      const dapps_1 = fixtureRegistryJson
        .dapps
        .filter((dapp) => {
    ...
    it('is able to filter results on or before certain list date', async () => {
      ...
      const registry_dapps_1 = await registry.dApps({listedOnOrBefore: endDate});
      const dapps_1 = fixtureRegistryJson
        .dapps
        .filter((dapp) => {
    ...
    it('is able to perform multiple filters', async () => {
      const language = 'hi';
      const age = 19;
      const registry_dapps_1 = await registry.dApps({language: language, minAge: age});
      const dapps_1 = fixtureRegistryJson.dapps.filter(dapp => dapp.language === language && dapp.minAge > age);
      registry_dapps_1.should.deep.equal(dapps_1);
    });
  });
});
